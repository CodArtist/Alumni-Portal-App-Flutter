...
class Internships extends StatefulWidget {
  ...
  var listIsLoading=true;
    ...
    var filteredList=[];
  Internships(this.internshipList,this.userList,this.listIsLoading)
  {
...
void initState() {
    super.initState();
    setState(() {
    widget.filteredList=widget.internshipList;
    });
    print("filtered");
...
//   widget.filteredList=internshipList;
//   listIsLoading=false;
// });
...
{var filteredlist=[];
  ...
  for(var i=0; i<widget.internshipList.length;i++)
  {
   if(widget.internshipList[i]['branch']==branches.indexOf(dropdownValueBranch).toString())
   {print("stage 1");
     if(dropdownValueTime=="1-5 months"&&(int.parse(widget.internshipList[i]['worktime'])>=1&&int.parse(widget.internshipList[i]['worktime'])<=5))
    {print("stage 2");
      filteredlist.add(widget.internshipList[i]);
    
    }
    if(dropdownValueTime=="more than 5 months"&&int.parse(widget.internshipList[i]['worktime'])>=6)
    {print("stage 3");
      filteredlist.add(widget.internshipList[i]);
    }
   }
  ...
  setState(() {
    widget.filteredList=filteredlist;
  });
}
       ...
       home: Scaffold(
        ...
        body: SafeArea(
          ...
          FlatButton(
            ...
            widget.filteredList.length!=0||widget.listIsLoading?ListView.builder(
             physics: NeverScrollableScrollPhysics(),
                shrinkWrap: true,
                itemCount:widget.listIsLoading?3:widget.filteredList.length,
                itemBuilder: (context,index){
                  return widget.listIsLoading?ShimmerLayout():InternshipCard(widget.filteredList[index],widget.userList);
                }
